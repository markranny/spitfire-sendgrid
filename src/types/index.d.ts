declare global {
  ...
  interface FlightLogResponse {
    success: boolean;
    headers?: string[];
    columnMappingSuggestions?: ColumnMappingSuggestions;
    rows?: string[][];
    message?: string;
  }

  interface ProcessFlightLogParams {
    files: File[];
  }
  ...
  interface WizardStep<T = any> {
    step: number;
    data: T;
    completed: boolean;
  }
  type WizardSteps<T = any> = WizardStep<T>[];
  ...
  interface CreateFlightLogsResponse {
    success: boolean;
    error?: string;
  }

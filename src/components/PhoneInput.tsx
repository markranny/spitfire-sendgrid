...
const InputComponent = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(
  ({ className, ...props }, ref) => (
    <Input className={cn("rounded-e-sm rounded-s-none min-h-12 border-gray-400" , className)} {...props} ref={ref} />
  )
);
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen} modal>
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant="outline"
          className="flex gap-1 rounded-e-none rounded-s-sm border-gray-400  border-r-0 px-3 focus:z-10 min-h-12"
          disabled={disabled}
        >
      ...
      <PopoverContent className="w-[300px] p-0 bg-white-50">
        <Command>
          <CommandInput
            value={searchValue}
...
const CountrySelectOption = ({
  ...
  return (
    <CommandItem className="gap-2 bg-white-50" onSelect={handleSelect}>

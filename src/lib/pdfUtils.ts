...
const addProfessionalExperience = async (
    ...
    for (const exp of experience) {
        if (exp.company && exp.job_title && exp.date_from && exp.date_to) {
            // Format text
            const companyPositionText = `${exp.company}, ${exp.job_title} - ${exp.job_location}`;
            const employmentDateText = `${formatDate(exp.date_from)} - ${formatDate(exp.date_to)}`;
            ...
            if (exp.keyrole) yPosition = drawBulletPoint(page, exp.keyrole, 60, yPosition, customFont, 11);
...
export const drawEducationSection = async (
    ...
    if (education) {
         ...
         const educationText = `${education.degree}, ${education.school_name}`;
         page.drawText(educationText, {
          ...
          let gpaTextWidth = 0;
          if (education.gpa) {
              const gpaText = `- GPA ${education.gpa}`;
          ...
          if (education.location) {
              page.drawText(education.location, {
                  x: currentX,
                  y: yPosition,
                  size: 11,
                  font: customFont,
        ...
        if (education.honors) {
            page.drawText(`Educational Honors: ${education.honors}`, {
                x: 60,
                y: yPosition,
                size: 11,
                font: customFont,
